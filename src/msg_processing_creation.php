...
$msg_processing_creation_handlers = array(
    ...
    GAME_STATE_REG_CHANNEL => function($context) {
        $handler_func = function($context, $channel_name) {
            ...
            if($result === 'invalid') {
                ...
                $context->comm->reply(
                    "I could not write to the channel. Make sure I have been added as an administrator (check out the <a href=\"https://eucompass.eu/Setting-up-a-public-channel\">online guide</a> for more information). Try again or provide a different channel name.",
                    null,
                    array("reply_markup" => array(
                        "inline_keyboard" => array(
                            array(
    ...
    GAME_STATE_LOCATIONS => function($context) {
        ...
        if($context->message) {
            ...
            if($context->message->is_location()) {
                ...
                if(preg_match_all('/(\d*.\d*)\s*,\s*(\d*.\d*)/s', $context->message->text, $matches, PREG_PATTERN_ORDER) >= 1) {
                    ...
                    // Generic text, used as location name
                    $context->memory[MEMORY_CREATION_LOCATION_NAME] = $context->message->text;
                }
            }
        }
        else if($context->callback) {
            if($context->callback->data === 'delete picture') {
                $context->memory[MEMORY_CREATION_LOCATION_PICTURE] = null;

                $context->comm->reply("Ok.");
            }
            else if($context->callback->data === 'another') {
        ...
        if(!isset($context->memory[MEMORY_CREATION_LOCATION_LAT])) {
            ...
            $reply_text .= "\n" . "All set? Send in a picture, a different position, or a different name to update, otherwise tap on <i>Save</i>.";
        }
...
$msg_processing_creation_state_entry = array(
    ...
    GAME_STATE_REG_CHANNEL => function($context) {
        $context->comm->reply(
            "Ok, name set. Please provide the name of <i>public Telegram channel</i> you will be using during your game.\nCheck out the <a href=\"https://eucompass.eu/Setting-up-a-public-channel\">online guide</a> for more information.",
            null,
            array("reply_markup" => array(
                "inline_keyboard" => array(
                    array(
    ...
    GAME_STATE_READY => function($context) {
        $context->comm->reply("Your game ‘%GAME_NAME%’ is now ready to be activated. Check out the <a href=\"https://eucompass.eu/create\">guide for game administrators</a> for further information. Good luck and have fun! ✌");
